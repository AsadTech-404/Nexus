  ...
  try {
    ...
    const collaborationsWithUserDetails = await Promise.all(
      collaborations.map(async (collab) => {
        const otherUserId =
          role === "investor" ? collab.entrepreneurId : collab.investorId;
        const otherUser = await User.findById(otherUserId)
          .select("id name email role avatarUrl isOnline")
          .lean();
    ...
    return res
      ...
      .json({
        success: true,
        collaborations: collaborationsWithUserDetails || [],
      });
  } catch (error) {
    console.error(error);
    return res
...
export const createCollaborationRequest = async (req, res) => {
  try {
    ...
    const notification = new Notification({
      ...
      message: `Request from ${req.user.name}`,
    ...
    res.status(201).json({
      ...
      collaborationRequest: newRequest,
    });
  } catch (error) {
    console.error(error);
